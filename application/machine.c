...
#define MACHINE_TEST_SPEED_MM_S_X100   150U

typedef enum
{
    MACHINE_TEST_STATE_IDLE = 0,        // 空闲状态，一般用于测试完成或启动失败
    MACHINE_TEST_STATE_MOTOR_B_START,   // 启动电机 B 移动 3cm
...
void MachineControl(void)
{
    ...
    case MACHINE_TEST_STATE_MOTOR_B_START:
         ...
         */
        if (StepMotor_RunDistanceMmX100(STEP_MOTOR_ID_B,
                                        STEP_MOTOR_DIR_PUSH,
    ...
    case MACHINE_TEST_STATE_MOTOR_B_WAIT:
        /*
         * 不使用延时等待，而是读取步进驱动状态。
         * 电机 B 达到目标步数后，step_motor 会在 TIM2 中断中自动停止 PWM。
         */
